...
import { DIAGNOSTIC_CENTER_CATEGORIES } from '../data/mockData';
...
export default function DiagnosticsSection({ onSelectCategory }) {
  ...
  useEffect(() => {
    ...
    api.getDiagnosticCenterCategories()
      ...
      .catch(() => {
        if (isMounted) {
          const filtered = DIAGNOSTIC_CENTER_CATEGORIES.filter(c => c.id !== 'all' && c.name !== 'By Specialization' && c.name !== 'By Ownership & Type');
  ...
  const displayCategories = categories.length > 0 ? categories : DIAGNOSTIC_CENTER_CATEGORIES.filter(c => c.id !== 'all' && c.name !== 'By Specialization' && c.name !== 'By Ownership & Type');
