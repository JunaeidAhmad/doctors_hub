...
export default function HospitalDetailPage({ hospitalId, onBookDoctorSlot, onBookLabTest, onNavigateHome, onNavigateHospitals }) {
  ...
  useEffect(() => {
    ...
    async function loadHospital() {
      ...
      const foundMock = [].find(h => h.id === hospitalId || h.slug === hospitalId)
        || [].find(c => c.id === hospitalId || c.slug === hospitalId)
        || [][0]
        || [][0];
      ...
      if (isMounted) {
        ...
        const filteredMockTests = [].filter(bt => bt.branch_id === hospitalId || bt.branch_id === foundMock.id);
        setBranchTests(filteredMockTests.length > 0 ? filteredMockTests : []);
        setLoading(false);
      }
    }
    loadHospital();
  ...
  const allDoctors = rawDoctors.length > 0
    ? rawDoctors
    : rawAffiliated.map(aff => {
        const docObj = (aff.doctor && typeof aff.doctor === 'object') ? aff.doctor : {};
        return {
  ...
  const testCategories = ['All', ...new Set(branchTests.map(bt => {
    ...
    return tObj.category_name || (typeof tObj.category === 'object' ? tObj.category.name : (tObj.category || 'General'));
  }))];

  const filteredTests = branchTests.filter(bt => {
    const tObj = bt.test_details || {};
    const tName = tObj.name || '';
    const tCat = tObj.category_name || (typeof tObj.category === 'object' ? tObj.category.name : (tObj.category || ''));
                  ...
                  return (
                    <div key={doc.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs flex flex-col justify-between space-y-4 hover:border-emerald-300 transition-colors">
                      ...
                      <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                        ...
                        <button
                          onClick={() => onBookDoctorSlot && onBookDoctorSlot(hospital, {
                            ...doc,
                            selectedSlot,
                            fee
                          })}
                ...
                {filteredTests.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {filteredTests.map((bt) => {
                      const testObj = bt.test_details || [][0];
                        ...
                        <div key={bt.id} className="bg-white p-5 rounded-2xl border border-slate-200 flex flex-col justify-between space-y-4 hover:border-emerald-300 transition-colors">
                          <div>
                            <span className="text-[10px] font-bold text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded uppercase">
                              {testObj.category_name || (typeof testObj.category === 'object' ? testObj.category.name : (testObj.category || 'Test'))}
                            </span>
